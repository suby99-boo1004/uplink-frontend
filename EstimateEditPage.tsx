...
import EstimateRegisterModal, {
  DraftEstimateCreatePayload,
} from "./Components/EstimateRegisterModal";
    ...
    sections: sections
      .map((s: any) => ({
        ...
        lines: (Array.isArray(s?.lines) ? s.lines : []).map((l: any) => ({
          ...
          source_id:
            l?.source_id === null || l?.source_id === undefined
              ? null
              : Number(l.source_id),
...
export default function EstimateEditPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // http(baseURL="/api")이므로 "/estimates/ID"로 호출
  ...
  async function handleSubmit(payload: DraftEstimateCreatePayload) {
    ...
    try {
      ...
      const body = res.data;
      const newId =
        Number(body?.estimate_id || body?.id || body?.estimateId || 0) || estimateId;
      nav(`/estimates/${newId}`);
    } catch (e: any) {
      const msg =
        e?.response?.data?.detail ||
  ...
  return (
    <div style={{ padding: 16 }}>
      ...
      {state.status === "error" && (
        <div style={{ color: "#FCA5A5", padding: 12 }}>{state.message}</div>
      )}
